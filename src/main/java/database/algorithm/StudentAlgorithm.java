...
public class StudentAlgorithm implements Comparable<StudentAlgorithm> {
    ...
    public int getPreference() {
        for(int prefer = 0; prefer < getApplyCount(); prefer++) {
            if(preferred.get(prefer).equals(matchedDepartment)) { 
                return StudentAlgorithm.MAX_APPLY - prefer;
            }
        }
        return 0;
    }
    ...
    @Override
    public int compareTo(StudentAlgorithm o) { // 학점이 높은 순으로 비교
